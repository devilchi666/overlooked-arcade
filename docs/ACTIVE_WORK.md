  ...
  - **Boundary enforcement track ✅ MERGED to main 2026-06-09**
    ...
    - **Phase 4 — typed `platform/api/` Tauri bridge — IN FLIGHT
      ...
      **[PLANS/theming-platform-api-bridge.md](PLANS/theming-platform-api-bridge.md)**.
      ...
      - **ARC 2 — Per-System Layout Substrate — L1 MERGED + L2a/L2b shipped 2026-06-15.**
        **L2a (view/layout manifest contract) ✅ shipped, CI-green** (D37):
        `ViewType`/`LayoutPrimitive` enums + `views?` manifest field + validator;
        no consumer. **L2b (D34 migration) ✅ shipped on
        `feat/theming-arc2-l2b-systemuiconfigs-migration` (stacks on L2a),
        pending visual-identical playtest** (D38): experiential per-system config
        → `themes/retroverse/` via `ThemePackage.perSystemUiConfigs` + App bridge;
        `touchInputSupported` split to platform-factual `systemSupportsTouch`;
        `uiConfigFor` merges over `BASELINE_UI`. typecheck/lint/vitest(131)/build
        green. **Merge L2a+L2b together after playtest. Then L3** (resolver +
        persisted user override that consumes the `views` contract).
