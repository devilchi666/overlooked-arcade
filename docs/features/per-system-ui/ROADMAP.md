...
| 4 | Boot animation framework + "Boot animations" Settings sub-toggle. New `SystemBootAnimation` component triggered by `activeSystemId` transition (sidebar-driven system entry). Full path runs `oa-boot-fade` CSS keyframe over 1 s with radial gradient tinted by `--color-system-accent`; compressed path (sub-toggle off OR `prefers-reduced-motion`) collapses to 200 ms. Dispatches per-system `boot-intro` SFX on the full path. Skippable on any input (mouse / keyboard / gamepad). | `feat/per-system-ui-stage-1-slice-4` | shipping |
