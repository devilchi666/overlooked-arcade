...
| `views` | optional — **per-view layout map** (ARC 2 L2/L3 / D32): `{ [view]: { layout?, per_system? } }`. `view` ∈ `manufacturer-browse` / `system-browse` / `game-browse` / `game-details`; `layout` (OPTIONAL, D40) ∈ `list` / `grid` / `carousel` / `wheel` / `custom` — the theme's view-wide default; `per_system` overrides it per SystemId. Resolution cascade (D32): user override → theme `per_system` → theme `layout` → the consumer's own default. **Live in `game-browse` (L3b):** a declared layout for the current system renders that primitive (`grid`→virtual grid, `list`→detail list; carousel/wheel/custom fall back to grid pending their renderers / L4 WheelNav); omit `layout` to keep the global capsule/list `viewMode` toggle for undeclared systems (the "coexist" model). Malformed → validator ERROR. |
