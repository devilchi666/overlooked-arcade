...
| `views` | optional — **per-view layout map** (ARC 2 L2/L3 / D32): `{ [view]: { layout?, per_system? } }`. `view` ∈ `manufacturer-browse` / `system-browse` / `game-browse` / `game-details`; `layout` (OPTIONAL, D40) ∈ `list` / `grid` / `carousel` / `wheel` / `custom` — the theme's view-wide default; `per_system` overrides it per SystemId. Resolution cascade (D32): user override → theme `per_system` → theme `layout` → the consumer's own default. **Live in `game-browse` (L3b/L4a):** a declared layout for the current system renders that primitive — `grid`→virtual grid, `list`→detail list, `carousel`→coverflow (L4a); `wheel`/`custom` still fall back to grid (`wheel` = L4b's radial primitive). Omit `layout` to keep the global capsule/list `viewMode` toggle for undeclared systems (the "coexist" model). Malformed → validator ERROR. |
