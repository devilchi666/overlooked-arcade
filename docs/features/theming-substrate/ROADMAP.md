...
| 2 | Platform / Theme SDK foundation. Move shared layer to `frontend/src/platform/` (`@oa/platform` alias). Define `theme.toml` manifest schema + `ThemeContext` interface. Rename Retroverse* → Theme* generics. ESLint boundary rule. Cleanup `HOTSPOT_SYSTEMS` triplicate + `customComponent` orphan. | (multi-slice) | ⬜ Slice A ✅ shipped 2026-06-06 (alias + tsconfig paths + barrel; HOTSPOT_SYSTEMS triplicate collapsed via `touchInputSupported` on SystemUIConfig; `customComponent` orphan deleted; settingsDialog + SettingsDialogs.tsx dead-letter deleted; residual 10 dialog signals migrated to platform/dialogs.ts; 744 tests pass; typecheck silent). Slice B pending = store moves + ThemeContext rename + ESLint boundary + Theme manifest. |
