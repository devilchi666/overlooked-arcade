...
**Status as of 2026-05-22:** Several specifics in this audit have shipped or drifted (PerSystemSettingsPage no longer exists, SettingsPage is down to 2 tabs and has been renamed LibraryManagerPage, PerGameSettingsDrawer is 10 tabs not 11, etc.). See sibling `UI_POLISH_PLAN.md` §0 for the current state. The structural-IA findings here (overlap matrix, orphaned features) remain valid as design context.
