...
- [features/hw-render/](features/hw-render/) — HW-render pipeline. Hosts GPU-rendered libretro cores (Dolphin, paraLLEl-N64, Beetle PSX HW, …) via the libretro HW render interface on a Vulkan-first backend abstraction. **M1 proven, M2 merged (zero-copy 60fps); M3 stranded on branch.** Design at [PLANS/hw-render-pipeline.md](PLANS/hw-render-pipeline.md).
...
- [features/theming-substrate/](features/theming-substrate/) — Theming substrate (BigBox-style themes, engine vs theme territory inside one binary). **ARC 1 complete** bar the `.oatheme` loader; **ARC 2 (Per-System Layout Substrate) planned 2026-06-15**. Renumbered to 4 arcs (D35): 1 MVS · 2 layout · 3 cinematic/scripting · 4 Theme Studio. Designs at [PLANS/theming-substrate.md](PLANS/theming-substrate.md) + [PLANS/theming-arc-2-per-system-layout.md](PLANS/theming-arc-2-per-system-layout.md).
...
- [PLANS/hw-render-pipeline.md](PLANS/hw-render-pipeline.md) — **engine arc** (2026-06-07). Libretro HW render interface so GPU-rendered cores run in-process. Vulkan-first multi-backend abstraction; zero-copy shared-device. **M1 proven, M2 merged; M3 stranded on branch.** Slotted before Theming ARC 3 (Cinematic & Scripting).
