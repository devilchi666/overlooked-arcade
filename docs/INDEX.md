...
- [features/unified-nav-tree/](features/unified-nav-tree/) — **Navigation Structure** (re-anchored 2026-06-19 from the theme-led "Unified Navigation Tree" framing). A **Settings-level, user-authored** tree (systems · groups · smart lists · curated collections · folders), nested any depth, each node's view selectable + remembered — to rival and *outdo* BigBox. Themes only *style* views; the structure is the user's. **NOT** the same as `features/unified-nav/` (that's spatial/input nav; this is the nav *content model*). 🔑 Vision + BigBox research + drift history: [features/unified-nav-tree/RESEARCH.md](features/unified-nav-tree/RESEARCH.md) (read first; the anti-drift source of truth); styling-cascade reference: [VIEW_MODEL.md](features/unified-nav-tree/VIEW_MODEL.md). Plan (7-slice program): [PLANS/unified-nav-tree.md](PLANS/unified-nav-tree.md); decisions NT1–**NT7**. Substrate: Slice-1 merged, Slice-2 green on branch (merges as plumbing); **Arc Slice 1 (collections & smart-lists as real authored nodes) queued** in [NEXT.md](NEXT.md). Declarative Showcase S3 resumes in Slice 7.
...
- [PLANS/unified-nav-tree.md](PLANS/unified-nav-tree.md) — **Unified Navigation Tree** (NEW arc, 2026-06-18). Navigation = one user-authored tree of nodes (system / group / collection / filter), each rendered by a per-node view (BigBox model, declaratively). Keystone = generalize node resolution from systems-only to systems-OR-game-set. Read the [VIEW_MODEL.md](features/unified-nav-tree/VIEW_MODEL.md) first. Slice 1 queued in NEXT.md HIGH band. Distinct from unified-navigation-and-panels (content model vs spatial/input nav).
