...
- [features/unified-nav-tree/](features/unified-nav-tree/) — **Unified Navigation Tree** (NEW arc, operator-approved 2026-06-18). One user-authored tree of nodes (system / group / collection / filter), each rendered by a **per-node view** (the BigBox per-level-per-node, change-on-the-fly, remembered model, built declaratively). Reunites the systems-only `views` sidebar tree + the separate flat `Collections` tab; same foundation that fixes "file themes can't do per-system views." **NOT** the same as `features/unified-nav/` (that's spatial/input nav; this is the nav *content model*). 🔑 Authoritative view model: [features/unified-nav-tree/VIEW_MODEL.md](features/unified-nav-tree/VIEW_MODEL.md) (anti-drift artifact). Plan: [PLANS/unified-nav-tree.md](PLANS/unified-nav-tree.md); decisions NT1–NT3. Slice 1 (keystone) queued in [NEXT.md](NEXT.md) HIGH band. Supersedes Declarative Showcase S3 (parks → resumes in Slice 5).
