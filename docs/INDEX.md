...
- [features/per-system-hub/](features/per-system-hub/) — **Per-System Settings Hub** (planned + S1 shipped 2026-06-14). Consolidates all per-system settings into one card-based **Systems** hub (systems grid → domain cards → editor), replacing the scattered Per-system / Media / Metadata categories + the Library→Game-media grid + BIOS. Built on the spatial-nav engine; delivers the unified-nav Pillar-B panel primitives. Design at [PLANS/per-system-settings-hub.md](PLANS/per-system-settings-hub.md).
...
- [PLANS/settings-ia-redesign.md](PLANS/settings-ia-redesign.md) — **Settings IA Redesign** (2026-06-14). Re-cuts the engine Settings IA around user intent (Themes/Appearance · Library · Organize My Collection · Import & Setup · External Emulators); declarative per-theme appearance schema; Library directory re-point. 5 slices; Slice 1 (re-skeleton + Library/Organize split, frontend-only) in NEXT.md HIGH band.
