...
- ✅ Dedicated `vb-monochrome` shader — shipped 2026-05-30 on `feat/virtualboy-completion-pack`. New `ShaderPreset::VbMonochrome` (id=6) with vertical scanline darken (mimics the spinning-mirror LED column scanner), soft circular vignette (eyepiece framing), and pure-red palette enforcement (crushes residual G/B). Single-pass — branches in `blit.wgsl`. New `shaders/presets/vb-monochrome.preset.toml`; virtualboy `defaultShaderPreset` flipped `plain` → `vb-monochrome`. Operator design locked: vertical scanlines + soft vignette + red-only.
