...
- ⬜ **Light gun support** — operator validation. Full pipeline wired as of 2026-06-05: LIGHTGUN dispatch (`feat/light-gun-harness` 2026-05-25) + IS_OFFSCREEN reload-by-aim (2026-05-27) + pointer mirrored to ports 1–4 (2026-06-05 `ee0f813`, lets a gun on either coop port see the OS mouse) + per-game Input dialog reads Beetle PSX's own `RETRO_ENVIRONMENT_SET_CONTROLLER_INFO` advertisement (dynamic-controller-info arc, `docs/PLANS/dynamic-controller-info.md`) so the dropdown shows "Guncon / G-Con 45" with id 260 (`SUBCLASS(LIGHTGUN, 0)`) and "Justifier" with id 516 (`SUBCLASS(LIGHTGUN, 1)`) — both correct subclass-encoded values straight from Beetle PSX rather than the broken generic `Light Gun = 4`. Time Crisis-style reload-by-aiming-off-screen wired via IS_OFFSCREEN. Flagship validation: Time Crisis 1/2, Point Blank trilogy, Lethal Enforcers, Crypt Killer — same code path as the validated NES Zapper case.
