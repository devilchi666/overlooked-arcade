...
- ✅ ScummVM auto-detect — shipped 2026-05-24 via the curated sentinel-filename approach (no standalone `scummvm.exe` dependency). New `apps/oa-shell/src/scummvm_detect.rs` module ships a table of ~18 well-known SCUMM games + ScummVM freewares (Monkey Island 1+2, DOTT, Sam & Max, Full Throttle, Dig, COMI, Indy 3+4, Loom CD, Zak Enhanced, Sky engine for BASS, Queen engine for FOTAQ, Lure, Drascula, Soltys). New `ScummvmDetectDialog` (opened from the Import Wizard's Step 2 banner when a scummvm rule is active) scans a parent folder, runs sentinel detection on each subdirectory, lets the operator confirm + edit detections, and writes `.scummvm` descriptor files in bulk. Games outside the curated set surface with `Not recognized — fill in gameid:engineid manually` so operators can still complete the table by hand without leaving the dialog.
