...
- ⬜ **Light gun support** (House of the Dead 2, Confidential Mission, Death Crimson OX, Maze of the Kings) — operator validation. Full pipeline wired as of 2026-06-05: LIGHTGUN dispatch (`feat/light-gun-harness` 2026-05-25) + IS_OFFSCREEN reload-by-aim (2026-05-27) + pointer mirrored to ports 1–4 (2026-06-05 `ee0f813`) + per-game Input dialog reads Flycast's own `RETRO_ENVIRONMENT_SET_CONTROLLER_INFO` advertisement (dynamic-controller-info arc, `docs/PLANS/dynamic-controller-info.md`) so the dropdown shows Flycast's authored light-gun labels with the correct subclass-encoded ids rather than the broken generic `Light Gun = 4`. Flagship validation: House of the Dead 2, Confidential Mission, Death Crimson OX, Maze of the Kings — same code path as the validated NES Zapper case.
