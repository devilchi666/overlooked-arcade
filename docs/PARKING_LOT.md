...
- 2026-05-24 — ScummVM `--detect` auto-generation of `.scummvm` files
  Why it came up: LaunchBox's ScummVM importer shells out to standalone ScummVM's `--detect` CLI, which fingerprints a directory of game data and emits the right `gameid:engineid` descriptor automatically. OA currently expects operators (or LaunchBox's importer) to create `.scummvm` files manually; auto-generation would lower the activation energy for fresh installs.
  Why deferred: orthogonal to the libretro-core wiring that just shipped 2026-05-24. Adding it would have widened the merged scope; the auto-detect UX is a polish layer on top of working infrastructure, not coupled to it. Three viable approaches when picked up: (A) shell out to standalone ScummVM in operator's PATH or auto-detected install (LaunchBox's exact approach — one external dep, real ScummVM-team-maintained detection database); (B) curated sentinel-filename heuristic (e.g. `MONKEY.000` → `monkey:scumm`) for the top ~30 titles, zero external deps but limited coverage; (C) bundle a minimal detector (significant maintenance burden, ruled out). (A) is the recommendation when this gets picked up — pairs naturally with a "Detect ScummVM games" button in the Import Wizard.
