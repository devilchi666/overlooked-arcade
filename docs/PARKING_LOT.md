...
- ~~2026-05-19 — External drag-drop file import~~
  ...
  **2026-06-17 — ✅ REOPENED + WORKING.** A dev drag-drop probe (F10 bench, `frontend/src/dev/DragDropProbe.tsx`) proved Tauri's `onDragDropEvent` now delivers real OS paths for both files and folders. Root cause of the May failure was the documented wry hidden-window registration bug ([wry #1639](https://github.com/tauri-apps/wry/issues/1639) / [tauri #14643](https://github.com/tauri-apps/tauri/issues/14643)): WebView2 registers its drop target at creation and skips it for a window built `.visible(false)`. The ARC-3 M1 `oa://window-shown` handshake means the window is now shown before any drop arrives, so the target IS registered — the bug no longer triggers (fix wry PR #1638 is still unmerged / not in our wry 0.55.1, so we rely on showing-before-drop, not the upstream fix). Shipped: multi-path drop ingest + dropped-file→parent-folder (`classify_drop_path`) + stale "unreliable" messaging removed in `App.tsx`. The DOM-layer/overlay-window workarounds were the wrong tree — Tauri's OS handler intercepts before the DOM (probe measured zero DOM drag events).
...
- 2026-06-15 — Cross-theme layout-override convenience: "Copy from theme…" + "Set as default for all themes" (Theming ARC 2 L5 follow-on) — **WE DO WANT THESE**
  Why it came up: L5 (D43) ships the per-system layout override UI keyed by `(theme, system, view)` — overrides are intentionally **theme-scoped** (D39: a layout that suits one theme's chrome may look wrong in another's; per-theme settings/palettes already work this way). The flip side is friction: a user who wants e.g. NES→list under every theme must re-pick it once per theme, and the L5 editor only edits the *active* theme.
  What (operator confirmed wanted, two affordances): (1) **"Copy from theme…"** — seed this theme's per-system layout picks from another theme's, so a user gets a head start without redoing them (keeps isolation; just a bulk copy into the active theme's slice of the override store). (2) **"Set as default for all themes" / "Set all"** — write the chosen view for this system across every theme's slice in one action (the user-global gesture without adding a new cascade tier). A possible cleaner alternative noted at design time was a real **global "default layout per system" tier** below the per-theme one (picks apply everywhere unless a theme-specific override exists) — more power but a 4th cascade tier to reason about; the two button affordances above achieve the practical goal without that complexity.
  Why deferred: L5's gate only needs per-theme-per-system to work, and it does; these are convenience layers on top. Park as a focused follow-on (a small editor addition writing the existing store — no schema/cascade change for the button forms).
  Reconsider when: playtest shows real re-picking friction across themes, or when L6/Theme-Studio surfaces make multi-theme curation common. Cross-ref: `frontend/src/engine/systemsHub/domains/LayoutEditor.tsx`, `platform/theme/layoutOverrides.ts`, theming DECISIONS D39/D43.
