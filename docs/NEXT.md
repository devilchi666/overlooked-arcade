...
### Theming ARC 2 — L1…L5 + P ✅ MERGED to main; L6 PARKED (ARC 2 complete)
...
- **P — `.oatheme` runtime loader (declarative-first)** — ✅ **P.1 SHIPPED + MERGED to main 2026-06-16** (the last open ARC-2 + ARC-1 thread, now closed). **Direction A (declarative-only)** ; plan [PLANS/theming-oatheme-loader.md](PLANS/theming-oatheme-loader.md); decisions D45–D49. Runtime themes are data, no author JS (PD1): a built-in `DeclarativeShell` renders `theme.toml` + tokens via the ARC 2 layout primitives/resolver; themes live at `themes/community/<id>/` (exe-dir or source-tree) and distribute as the **`themes` pack type**. Custom-JS runtime loading + the CSP allowlist remain **deferred to P.2** (tees up ARC 3 Rhai).
  - **P.1 S1 ✅ MERGED** — Rust loader + `oa_themes_list_disk` (`theme_loader.rs`).
  - **P.1 S2 ✅ MERGED** — `DeclarativeShell` + `diskThemeToPackage` + `bare-declarative` dogfood (operator A/B'd vs `bare`).
  ...
  - **named preset registry + contract wiring ✅ MERGED to main** (`b0ca076`;
    ...
    - ✅ **Wiring tail MERGED to main** (`60af185`; branch `feat/motion-wire-consumers`,
      ...
      2. ✅ **`declarativeShell` migrated** onto `SpecTransition` +
         ...
         get choreography without custom render code) **MERGED to main 2026-06-18**
         (`feat/motion-selection-ambient-hook`; operator playtested): a `SelectionMotion`
         wrapper (`platform/theme/SelectionMotion.tsx`, D58.10) the `declarativeShell`
         wraps every card in — focused-item pop (cancel-on-defocus, `skipInitial`) + one
         ambient loop on the focused item. Exercised on the on-disk **`aurora`** showcase
         (`bare-declarative` kept as the minimal floor) — see the **Declarative Showcase**
         arc below.
