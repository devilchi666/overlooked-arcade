...
- **P — `.oatheme` runtime loader (declarative-first)** — the last open ARC-2 thread (also the last open ARC-1 thread). **Direction A (declarative-only) locked 2026-06-16** + planned: [PLANS/theming-oatheme-loader.md](PLANS/theming-oatheme-loader.md). Runtime themes are **data, no author JS** (PD1): a built-in `DeclarativeShell` renders `theme.toml` + tokens + assets via the ARC 2 layout primitives/resolver; themes live at `<exe_dir>/themes/community/<id>/` and distribute as the **`themes` pack type** on the channel shipped 2026-06-16 (PD2–PD4). Custom-JS runtime loading + the CSP allowlist are **deferred to P.2** (tees up ARC 3 Rhai). Doesn't depend on L6.
  - **P.1 Slice 1 ⬜ QUEUED — Rust theme loader + discovery command (no UI, no rendering).** New `apps/oa-shell/src/theme_loader.rs`: define the `theme.toml` (+ optional `tokens.toml`) serde schema mirroring the TS declarative `ThemeManifest`/`ThemeTokens` surface; discover `<exe_dir>/themes/community/<id>/` (reserve a loose-folder dev path); parse TOML → `Vec<DiskThemeDescriptor>` (manifest + tokens + per-system + base path); skip-on-malformed + logged; Tauri command `oa_themes_list_disk`. Unit tests (parse good/bad, skip malformed, base-path). No frontend consumer yet. **Acceptance:** a hand-placed `<exe_dir>/themes/community/foo/theme.toml` appears in `oa_themes_list_disk`; a malformed one is skipped. Then S2 (`DeclarativeShell` generic theme) → S3 (register disk themes + Appearance picker + `themes` pack type).
